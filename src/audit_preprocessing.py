# =====================================================================
# FILE: audit_preprocessing.py
# FINAL VERSION — PREPROCESSING QUALITY AUDIT (250Hz PIPELINE ONLY)
# =====================================================================
...
def compute_frequency_metrics(signal, fs=250.0):
    ...
    # Powerline noise (49–51Hz)
    idx_pl = np.where((freqs >= 49) & (freqs <= 51))[0]
    pl_energy = (
...
def compute_morphology_metrics(raw_sig, clean_sig):
    ...
    # Pearson Correlation
    try:
        corr, _ = pearsonr(raw_sig, clean_sig)
...
def audit_single_signal(raw_signal, clean_signal):
    ...
    metrics.update(
        compute_hrv_metrics(clean_signal, fs=FS)
    )
...
print("=" * 80)
print("🔬 PREPROCESSING QUALITY AUDIT — 250Hz RESAMPLED PIPELINE")
print("=" * 80)

manifest_path = os.path.join(
    cfg.RESAMPLE_BASE,
    "manifest_ptbxl_v5.csv"
)
...
# RAW -> CLEANED
# Seluruh eksperimen sekarang fokus pada final 250Hz
...
for pipeline_name, folders in PIPELINES.items():
    ...
    raw_dir = cfg.SUB_FOLDERS[folders["raw_folder"]]
    clean_dir = cfg.SUB_FOLDERS[folders["clean_folder"]]
    ...
    for cls_name in cfg.CLASS_NAMES:
        ...
        for _, row in tqdm(
            ...
            try:
                ...
                # Lead II dipilih sebagai referensi audit
...
csv_path = os.path.join(
    AUDIT_DIR,
    "preprocessing_audit_metrics.csv"
)
...
print("\n" + "=" * 80)
print("✅ PREPROCESSING AUDIT FINISHED")
print("=" * 80)